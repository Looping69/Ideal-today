...
export default function PricingPage() {
    ...
    useEffect(() => {
        const nextBilling = searchParams.get('billing') === 'annual' ? 'annual' : 'monthly';
        ...
        <div className="mx-auto max-w-7xl space-y-10 pb-16">
                ...
                <div className="relative grid gap-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-end">
                    <div className="space-y-6">
                        ...
                        <div className="flex flex-wrap gap-3">
                            ...
                            {audience === 'host' && (
                                <Button variant="outline" onClick={() => navigate("/host")} className="rounded-full border-slate-300 bg-white/80">
                                    Back to Host Workspace
                                </Button>
                            )}
                        </div>

                        <div className="inline-flex w-fit items-center gap-2 rounded-full border border-slate-200 bg-white/85 p-1 shadow-sm">
                            <button
                                type="button"
                                onClick={() => setBillingInterval('monthly')}
                                className={`rounded-full px-4 py-2 text-sm font-semibold transition-colors ${billingInterval === 'monthly' ? 'bg-slate-950 text-white' : 'text-slate-600 hover:text-slate-950'}`}
                            >
                                Monthly
                            </button>
                            <button
            ...
            <section className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm md:p-10">
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-200">
                    <div className="grid grid-cols-5 bg-slate-950 text-white">
                    ...
                    {comparisonRows.map((row, rowIndex) => (
                        <div key={row.label} className={`grid grid-cols-5 ${rowIndex % 2 === 0 ? 'bg-white' : 'bg-slate-50'}`}>
