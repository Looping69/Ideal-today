export const SETUP_SQL = `-- Enable required extensions
...
do $$ 
begin
  if not exists (select 1 from pg_policies where policyname = 'Everyone can read admin settings') then
    create policy "Everyone can read admin settings" on public.admin_settings for select using (true);
  end if;
  if not exists (select 1 from pg_policies where policyname = 'Admins can update admin settings') then
    create policy "Admins can update admin settings" on public.admin_settings for update using (
      exists (
